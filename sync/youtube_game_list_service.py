...
class YoutubeSyncService:
    ...
    async def get_youtube_game_videos_info(self, videos_id_list, game, today_timestamp, search_query):
        ...
        for video in videos_info:
            ...
            # channel_ids[video['id']] = video['snippet']['channelId']
        ...
        channels_info = await self.youtube_api_service.get_channels_info(channel_ids_str)

        for id, channel_info in enumerate(channels_info):
            videos[id]['subscribers_count'] = channel_info['statistics']['subscriberCount'] if 'subscriberCount' in \
                                                                                               channel_info[
    ...
    async def get_single_video_info(self, video, game, today_timestamp, search_query):
        ...
        return {
            ...
            "publish_time": int(time.mktime(
                datetime.datetime.strptime(video['snippet']['publishedAt'], "%Y-%m-%dT%H:%M:%SZ").timetuple())),
